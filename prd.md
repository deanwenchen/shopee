...
### 模块 4：商品详情页组件 ✅ 已完成
- ProductGallery - 图片轮播组件（支持滑动切换、圆点指示器、收藏按钮）
- ProductInfo - 商品信息组件（价格、折扣、促销倒计时）
- BottomBar - 底部操作栏组件（收藏、加入购物车、立即购买）
- FlashSaleBanner - 促销 Banner 组件
- SKUSelector - SKU 选择器 Bottom Sheet 组件（颜色/尺寸/数量）
...
### 10. ShopPage（商城主页）
- 顶部搜索栏
- 分类导航（Women/Men/Kids 等）
- Flash Sale 促销专区（限时折扣 + 倒计时）
- Most Popular 热门商品轮播
- You Might Like 推荐商品网格
- 商品点击跳转详情页 `/product/:id`

### 11. ProductDetail（商品详情页）
- **布局结构**:
  - 固定容器高度 100vh，内部滚动
  - 底部操作栏固定在容器底部（z-index: 100）
  - 内容区域可独立滚动，底部留白 100px 避免遮挡
  - Status Bar 固定在顶部（z-index: 1000）
- **Status Bar**:
  - 时间显示（9:41）
  - 电池图标（边框 + 电容量 + Cap）
  - WiFi 信号图标
...
- **图片轮播**:
  - 支持多张图片滑动切换（5+ 张）
  - 圆点指示器（当前选中为蓝色长条 40px，其他为圆形 10px）
  - 3 秒自动轮播
  - 浮动分享按钮（右上角，圆形粉色背景 + share 图标）
- **商品信息**:
  - 价格显示（$17,00，Raleway Bold 26px）
  - 商品描述（Nunito Sans 15px，行高 20px）
- **Variations 预览**:
  - 已选规格芯片（颜色、尺寸，灰色背景）
  - 展开按钮（蓝色圆形箭头图标）
  - 缩略图预览（3 个，80x80px，圆角 8px）
- **Specifications（规格）**:
  - Material 标签（Cotton 95%、Nylon 5%，粉色背景 #ffebeb）
  - Origin 标签（EU，蓝色背景 #e5ebfc）
  - Size guide 按钮（带箭头图标）
- **Delivery（配送）**:
  - Standart 选项（$3,00，5-7 days，蓝色边框，浅蓝背景天数标签）
  - Express 选项（$12,00，1-2 days，蓝色边框，浅蓝背景天数标签）
- **Rating & Reviews（评分与评价）**:
  - 评分汇总（4/5 星，大星标 + 蓝色背景评分值）
  - 用户评价（Veronika，头像 +4.5 星 + 评价内容）
  - View All Reviews 按钮（蓝色全宽按钮）
- **Most Popular（热门商品）**:
  - 标题 + See All 按钮（带箭头）
  - 横向滚动商品列表（4 个商品）
  - 商品卡片（104x140px，圆角 9px，阴影，图片 + Like 按钮 + 价格 + 标签）
  - 标签类型：New（粉色渐变）、Sale（红色渐变）、Hot（橙色渐变）
- **You Might Like（推荐商品）**:
  - 2 列网格布局
  - 商品卡片（圆角 9px，阴影，图片 + 商品名 + 价格）
- **底部操作栏**:
  - 收藏按钮（48px 宽，灰色背景）
  - Add to Cart 按钮（flex: 1，黑色背景）
  - Buy Now 按钮（flex: 1.5，蓝色背景）
  - Home Indicator（底部 134px 黑色圆角条）
- **SKU 选择器 Bottom Sheet**:
  - 弹窗从底部滑出（z-index: 99，低于 BottomBar）
  - 高度限制：`calc(100vh - 84px)`
  - 底部 padding: `calc(24px + 84px)` 避免被 BottomBar 遮挡
  - **颜色选择**: 4 个颜色选项（带图片预览，网格布局）
  - **尺寸选择**: S/M/L/XL/XXL/XXXL（6 个按钮，XXL/XXXL 可禁用）
  - **数量选择**: 圆形 +/- 按钮（最小值 1）
  - **实时选择**: 点击选择即生效，无需 Apply 按钮
  - **关闭方式**: 点击遮罩或关闭按钮
  - **确认操作**: 关闭弹窗后点击 BottomBar 的 Add to Cart / Buy Now
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| 3.3 | 2026-04-01 | 商品详情页完整实现：Status Bar、图片轮播（5+ 张，自动轮播）、Specifications（Material/Origin/Size guide）、Delivery（Standart/Express）、Rating & Reviews（4/5 星 + Veronika 评价）、Most Popular（4 个商品横向滚动）、You Might Like（2 列网格）；前端构建验证成功 |
| 3.2 | 2026-04-01 | 商品详情页布局修复：BottomBar 固定在容器底部（z-index: 100），内容区域独立滚动（padding-bottom: 100px），SKU 选择器弹窗高度限制 `calc(100vh - 84px)` 并添加底部留白；移除 Apply 按钮，选择实时生效 |
